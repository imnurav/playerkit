...
| Breakpoint        | Behavior                                                     |
| ----------------- | ------------------------------------------------------------ |
| > 760px (desktop) | Full control bar with all buttons, dropdown settings         |
| ≤ 760px (mobile)  | Compact controls, bottom sheet settings, some buttons hidden |
| Landscape mobile  | Sheet max-height adjusted                                    |

The `isMobile` prop from `@nurav/player-react` combines touch detection + screen width for accurate mobile identification.

## Styling

All styles use BEM naming with `vp-` prefix:
