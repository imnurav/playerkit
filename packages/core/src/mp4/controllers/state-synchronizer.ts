 ...
 * Offloads all direct video listener wiring from the Mp4Player facade.
 */
export class StateSynchronizer {
  ...
  private attachEvents(): void {
    this.on("loadedmetadata", () => {
      ...
      const pendingStartTime = this.callbacks.getPendingStartTime();
      if (pendingStartTime !== undefined) {
        this.callbacks.setVideoTime(pendingStartTime);
        this.callbacks.clearPendingStartTime();
      }
    });
